...
Now that there is more than one constructor in our variant it is no longer possible for us to get the value by destructuring like we have above, instead we handle this with a switch:
```reason
type result = | Ok(string) | Error;
...
```
If you are coming from JavaScript then you are probably already familiar with the switch statement, the variable goes in the top and then we write a handler for each possible value.
...
Or you can match whole variant types with a hash followed by the variant name:
```reason
let matchVariants = 
  fun
  | `One => "number"
