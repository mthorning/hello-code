...
This is where variants come into their own; the compiler will check that you have handled every possible case for any variants you declare. This removes a significant cognitive load and also makes refactoring code a lot less unlikely to lead to introducing bugs. It's not possible for us to handle multiple cases using destructuring like we have above, the way we handle this is with a switch:
```reason
type result = | Ok(string) | Error;
...
```
If you are coming from JavaScript then you are probably already familiar with the switch, the value goes in the top and then we write a case for each possibility.

Another standard variant type is option. Reason does not have null (no more "cannot read property 'x' of null!), it handles the possibility of a value not being there with the option variant which has two cases, Some and None:
```reason
